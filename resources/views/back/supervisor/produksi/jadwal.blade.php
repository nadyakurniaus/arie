...
<!-- modal start -->
<div class="modal fade modal-3d-sign" id="modalMP" aria-labelledby="exampleModalTitle" role="dialog" tabindex="-1"
    ...
    <div class="modal-dialog modal-simple">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <br>
                <div class="form-group">
                    <label for="tanggal_order" class="col-sm-4 control-label">Tanggal Masuk Produksi</label>
                    <div class="col-sm-6">
                        <input type="hidden" id="id_produksiC">
                        <input type="hidden" id="primaryC">
                        <input type="hidden" id="targetC">
                        <input type="text" name="qty_cuting" id="qty_cuting" class="form-control datepicker"
                             readonly/>
                    </div>
                </div>
            </div>
            <div class="modal-body">
                ...
                <div class="row offset-md-2"><div class="col-md-6"><label>Tanggal Masuk Finishing :</label></div><label id="Finishing">-</label></div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                <button type="button" id="simpanMP" class="btn btn-primary">Simpan</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function showTable() {
        ...
        dtTable = $(".table-user").DataTable({
            ...
            ajax: ({
                url: '{{ route("produksi.dtProduksiSPV") }}',
                type: "POST",
                data: data,
                dataType: "JSON",
                headers: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (data == '1') {
                            return '<button type="button" class="btn btn-icon btn-primary btn-masterplate" data-target="#modalMP" data-toggle="modal"><i class="icon wb-plugin" aria-hidden="true"></i></button>'+
                        '<button type="button" class="btn btn-icon btn-info" data-target="#modalDetail" data-toggle="modal"><i class="icon wb-info" aria-hidden="true"></i></button>'
                            ...
                            return '<button type="button" class="btn btn-icon btn-primary btn-masterplate" disabled><i class="icon wb-plugin" aria-hidden="true"></i></button>'+
                        '<button type="button" class="btn btn-icon btn-info" data-target="#modalDetail" data-toggle="modal"><i class="icon wb-info" aria-hidden="true"></i></button>'
                        }
                    }
                }
        ...
        $(".table-user tbody").on('click', 'tr button.btn.btn-masterplate', function() {
            var datas = dtTable.row($(this).parents('tr')).data();
            ...
            $('#simpanMP').click(() => {
            $.ajax({
                url: '{{ route("produksi.simpanDetail") }}',
                type: "POST",
                data: {
                    'id': $('#id_produksiC').val(),
                    'qty': $('#qty_cuting').val(),
            ...
            $.ajax({
                ...
                success: function(data) {
                    console.log(data)
                    $('#targetPrint').html(data[0].qty);
                    $('#cctc').html(data[1].qty);
                    $('#cctp').html(data[2].qty);
                    $('#qtyp').html(data[3].qty);
                }
            })
        });
            ...
            $('#thnamaproduk').html(datas.namaproduk);
             $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#cacat_cuting').html(jing);
                    var tot = moment(data[2].qty).format('DD/MM/YYYY');
                    $('#cacat_printing').html(tot);
                    var dan = moment(data[3].qty).format('DD/MM/YYYY');
                    $('#qty_finishing').html(dan);
                }
            })
        });
